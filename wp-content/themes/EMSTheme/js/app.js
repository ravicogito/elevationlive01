...
$(document).ready(function() {
    ...
    function s() {
        ...
        if ($("#ajax-show-info .btn").hide().attr("href", "Javascript:void(0);"), $("#miniload").show(), $("#quote-wrap").slideUp("fast"), $("#error_msg").css("display", "none"), $("#OneWayJourney").is(":checked")) var o = "single";
        else var o = "return";
        var _ = $("#spinner-url").val(),
            v = "<img src='" + _ + "' />";
        $("#data-5-express, #data-5-standard, #data-15-express, #data-15-standard, #data-30-express, #data-30-standard, #data-over30-express, #data-over30-standard").html(v), 
        $.when($.post(ajax_url, {
            from: e,
            to: s,
            journey: o,
            ...
            s.w30kg.hasOwnProperty("standard") ? (i = parseFloat(s.w30kg.standard.amount), $("#data-over30-standard").html(a + i.toFixed(2)), $("#standard-over30-button").attr("href", s.w30kg.standard.url)) : $(".standard-results").slideUp("fast"), p = parseFloat(s.w30kg.express.amount), $("#data-over30-express").html(a + p.toFixed(2)), $("#express-over30-button").attr("href", s.w30kg.express.url), $("#ajax-show-info .btn").show();

                 s.w5kg.hasOwnProperty("standard") ? $('.standard-results').show() : console.log('only express amount');
                s.w15kg.hasOwnProperty("standard") ? $('.standard-results').show() : console.log('only express amount');
                s.w30kg.hasOwnProperty("standard") ? $('.standard-results').show() : console.log('only express amount');
...
/*
    ...
    function similitude(a, b) {
        for (var i = 0, len = Math.max(a.length, b.length); i < len; i++){
            if ($.type(a.charAt(i)) !== $.type(b.charAt(i))) {
                return "false";//Math.round(i / len * 100);
            } else {
                return "correct";
            }
        }    
    ...
    $(".page-template-quotepage #Collection_Postcode").blur(function() {
            var e = $("#Collection_Postcode").val(),
            s = $("#Destination_Postcode").val(),
            f = $("#quote-Origin_CountryIso").val(),
            t = $("#quote-Destination_CountryIso").val();

            $.post(ajax_url, {
                from: f,
                to: t,
                collection_code: e,            
                action: "do_postcode_chk_collection"
            }, function(response) {
               var orgType = response['postcode'];
               var country_code = response['country_code'];
               var pattern =  response['country_postcode_pattern'];
               //alert(pattern);
               /*console.log(pattern);
               console.log(response['country_postcode_patt']);
               console.log(response['country_pcode']);
               
               console.log(response['toCountry']);
               console.log(response['fromCountry']);*/

                 if(pattern=='0'){
                 $(".success_color_collection").html("");
                    $(".error_color_collection").html("The Postcode is not in the correct format for the country selected. The postcode should be in the format A9 9AA, A99 9AA, A9A 9AA, AA9 9AA, AA99 9AA, AA9A 9AA");
               } else if(pattern=='2'){
                 $(".success_color_collection").html("");
                $(".error_color_collection").html("The Postcode is not in the correct format for the country selected.");

               } else{
                 $(".error_color_collection").html("");
                 $(".success_color_collection").html("Enter post codes for a more accurate delivery time and price. Without postcodes the quote assumes capital to capital.");
             }
               //var retType  = similitude(orgType, f);
            }, "json");
         });


    $(".page-template-quotepage #Destination_Postcode").blur(function() {
            var e = $("#Collection_Postcode").val(),
            s = $("#Destination_Postcode").val(),
            f = $("#quote-Origin_CountryIso").val(),
            t = $("#quote-Destination_CountryIso").val();
            ...
            $.post(ajax_url, {
                ...
                to: t,
                destination_code: s,            
                action: "do_postcode_chk_destination"
            }, function(response) {
               var orgType = response['postcode'];
               var country_code = response['country_code'];
               var pattern =  response['country_postcode_pattern'];
               ...
               console.log(response['fromCountry']);*/
                 ...
                 if(pattern=='0'){
                 $(".success_color_destination").html("");
                    $(".error_color_destination").html("The Postcode is not in the correct format for the country selected. The postcode should be in the format A9 9AA, A99 9AA, A9A 9AA, AA9 9AA, AA99 9AA, AA9A 9AA");
               } else if(pattern=='2'){
                 $(".success_color_destination").html("");
                $(".error_color_destination").html("The Postcode is not in the correct format for the country selected.");

               } else{
                 $(".error_color_destination").html("");
                 $(".success_color_destination").html("Enter post codes for a more accurate delivery time and price. Without postcodes the quote assumes capital to capital.");
             }
               //var retType  = similitude(orgType, f);
            }, "json");
         });
