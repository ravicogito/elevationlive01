...
// Retrieve label url and tracking number or error message
    ...
    public function send_transaction_email($parcels, $transactions, $order) {
		...
		<td align='left' bgcolor='#FFFFFF' style='background-color:#FFFFFF;'>" . number_format($remainder, 2) . " " . $currency . "</td>
			...
			North Yorkshire<br />
			HG1 2NE<br />
			VAT Number: 280 6981 72<br />
			Telephone: 01423 530995<br />
    ...
    public function do_quick_quote() {
        ...
        $results = $wpdb->get_results( "SELECT * FROM ems_shippoapi_data WHERE service_from_country_code='".$from_country_code."' AND service_to_country_code='".$to_country_code."'"); // Query to fetch data from database table and storing in $results
         ...
         if(!empty($results)){
                ...
                foreach($results as $row):
                    ...
                    $weight = 'w' . $row->weight;
                            ...
                            $markup = $this->get_markup_for_parcel_kg($row->service_from_country_code, $row->service_to_country_code, $row->weight, 'express');
                            ...
                            if ($journey == 'single'):
                                ...
                                $final_data[$row->weight . 'kg']['express']['currency'] = $currency;
                                $ow = str_replace('w', '', $row->weight);
                                $final_data[$row->weight . 'kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . number_format($row->amount * 2, 2) . "&amp;kg=" . $ow . "&amp;protocol=express";
                            endif;
                        endif;
                        ...
                        if ($row->servicelevel_name == 'ECONOMY SELECT DOC'):
                            ...
                            if ($journey == 'single'):
                                ...
                                $ow = str_replace('w', '', $row->weight);
                                $final_data[$row->weight . 'kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . $amount . "&amp;kg=" . $ow . "&amp;protocol=standard";
                            else:
                                $final_data[$row->weight . 'kg']['standard']['amount'] = number_format($amount * 2, 2);
                                $final_data[$row->weight . 'kg']['standard']['currency'] = $currency;
                                $ow = str_replace('w', '', $row->weight);
                                $final_data[$row->weight . 'kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . number_format($amount * 2, 2) . "&amp;kg=" . $ow . "&amp;protocol=standard";
                            endif;
                        endif;
                        ...
                        if ($row->servicelevel_name == 'DOMESTIC EXPRESS DOC' && $zone == 'domestic'):
                            ...
                            if ($journey == 'single'):
                                ...
                                if (isset($final_data[$row->weight . 'kg']['standard']['amount']) && $final_data[$rwo->weight . 'kg']['standard']['amount'] < $amount):
                                    continue;
                                endif;
                            else:
                         ...
                         if ($row->servicelevel_name == 'MEDICAL EXPRESS'):
                ...
                foreach ($weights as $weight):
					...
					if(!empty($rates)):
                        ...
                        if ($rate->servicelevel->name == 'ECONOMY SELECT DOC'):
                         ...
                         if ($rate->servicelevel->name == 'MEDICAL EXPRESS'):
                            $markup = $this->get_markup_for_parcel_kg($from_country_code, $to_country_code, $weight, 'standard');
