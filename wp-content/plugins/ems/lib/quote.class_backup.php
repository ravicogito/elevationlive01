...
// Retrieve label url and tracking number or error message
    ...
    public function do_quick_quote() {
        ...
        $results = $wpdb->get_results( "SELECT * FROM ems_shippoapi_data WHERE service_from_country_code='".$from_country_data[0]->country_code."' AND service_to_country_code='".$to_country_data[0]->country_code."'"); // Query to fetch data from database table and storing in $results
        if(!empty($results)) {
                ...
                foreach($results as $row):
                    ...
                    //foreach ($weights as $weight):
                        ...
                        if ($row->service_from_country_code == 'GB'): $carriers = array($this->uk_carrier);
                        else: $carriers = array($this->foreign_carrier);
                        endif;
                        ...
                        //$weight = 'w' . $weight;
                            ...
                            if ($row->servicelevel_name == 'EXPRESS WORLDWIDE DOC'):
                                $markup = $this->get_markup_for_parcel_kg($row->service_from_country_code, $row->service_to_country_code, $weight, 'express');
                                ...
                                if ($journey == 'single'):
                                    ...
                                    if (isset($final_data[$row->weight . 'kg']['express']['amount']) && $final_data[$row->weight . 'kg']['express']['amount'] < $amount):
                                        continue;
                                    endif;
                                else:

                                    $a2 = $amount * 2;
                                    if (isset($final_data[$row->weight . 'kg']['express']['amount']) && $final_data[$row->weight . 'kg']['express']['amount'] < $a2):
                                        continue;
                                    endif;
                                ...
                                if ($journey == 'single'):
                                    $final_data[$row->weight . 'kg']['express']['amount'] = $amount;
                                    $final_data[$row->weight . 'kg']['express']['currency'] = $currency;
                                    $ow = str_replace('w', '', $weight);
                                    $final_data[$row->weight . 'kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . $amount . "&amp;kg=" . $ow . "&amp;protocol=express";
                                else:
                                    $final_data[$row->weight . 'kg']['express']['amount'] = number_format($amount * 2, 2);
                                    $final_data[$row->weight . 'kg']['express']['currency'] = $currency;
                                    $ow = str_replace('w', '', $row->weight);
                                    $final_data[$row->weight . 'kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . number_format($amount * 2, 2) . "&amp;kg=" . $ow . "&amp;protocol=express";
                                endif;
                            endif;

                            if ($row->servicelevel_name == 'ECONOMY SELECT DOC'):
                                $markup = $this->get_markup_for_parcel_kg($row->service_from_country_code, $row->service_to_country_code, $weight, 'standard');
                                if ($rate->currency == 'GBP'):
                                    $amount = $rate->amount;
                                else:
                                    $amount = $rate->amount_local;
                                ...
                                if ($journey == 'single'):
                                    ...
                                    if (isset($final_data[$row->weight . 'kg']['standard']['amount']) && $final_data[$row->weight . 'kg']['standard']['amount'] < $amount):
                                        continue;
                                    endif;
                                else:

                                    $a2 = $amount * 2;
                                    if (isset($final_data[$row->weight . 'kg']['standard']['amount']) && $final_data[$row->weight . 'kg']['standard']['amount'] < $a2):
                                        continue;
                                    endif;
                                ...
                                if ($journey == 'single'):
                                    $final_data[$row->weight . 'kg']['standard']['amount'] = $amount;
                                    $final_data[$row->weight . 'kg']['standard']['currency'] = $currency;
                                    $ow = str_replace('w', '', $row->weight);
                                    $final_data[$weight . 'kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . $amount . "&amp;kg=" . $ow . "&amp;protocol=express";
                                else:
                                    $final_data[$row->weight . 'kg']['standard']['amount'] = number_format($amount * 2, 2);
                                    $final_data[$row->weight . 'kg']['standard']['currency'] = $currency;
                                    $ow = str_replace('w', '', $row->weight);
                                    $final_data[$row->weight . 'kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . number_format($amount * 2, 2) . "&amp;kg=" . $ow . "&amp;protocol=express";
                                endif;
                            endif;



                            if ($row->servicelevel_name == 'EXPRESS WORLDWIDE EU DOC'):
                            ...
                            if ($row->servicelevel_name == 'DOMESTIC EXPRESS DOC' && $zone == 'domestic'):

                                $attributes = $rate->attributes;


                                $markup = $this->get_markup_for_parcel_kg($row->service_from_country_code, $row->service_to_country_code, $row->weight, 'standard');
                                if ($row->currency == 'GBP'):
                                    $amount = $row->amount;
                                else:
                                    $amount = $row->amount_local;
                                ...
                                if ($journey == 'single'):
                                    ...
                                    $final_data[$row->weight . 'kg']['standard']['currency'] = $currency;
                                    $ow = str_replace('w', '', $weight);
                                    $final_data[$row->weight . 'kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $row->service_from_country_code . "&ct=" . $row->service_to_country_code . "&amp;pr=" . $amount . "&amp;kg=" . $ow . "&amp;protocol=express";
                                else:
                                    $final_data[$row->weight . 'kg']['standard']['amount'] = number_format($amount * 2, 2);
                                    $final_data[$row->weight . 'kg']['standard']['currency'] = $currency;
                                    $ow = str_replace('w', '', $row->weight);
                ...
                //endforeach;
                     ...
                     foreach ($weights as $weight):
                        ...
                        /*$wpdb->insert('ems_shippoapi_data', array('service_from_country_code' => $from_country_code, 'service_from_city' => $from_country1->city,
                                'service_from_postcode' => $from_country1->postcode,
                                'service_to_country_code' => $to_country_code,
                                'service_to_city' => $to_country1->city,
                                'service_to_postcode' => $to_country1->postcode,
                                'servicelevel_name' => $rate->servicelevel->name,
								'weight' => $weight,
                                'amount' => $rate->amount,
                            ...
                            if ($rate->servicelevel->name == 'ECONOMY SELECT DOC' && $zone='europe' ):
			 ...
			 if ($journey == 'single'):
		    $final_data['w5kg']['standard']['amount'] = '110.00';
			$final_data['w5kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w5kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=110.00&amp;kg=5&amp;protocol=standard";
			
			$final_data['w10kg']['standard']['amount'] = '297.00';
			$final_data['w10kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w10kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=297.00&amp;kg=10&amp;protocol=standard";
			
			$final_data['w15kg']['standard']['amount'] = '319.00';
			$final_data['w15kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w15kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=319.00&amp;kg=15&amp;protocol=standard";
			
			$final_data['w30kg']['standard']['amount'] = '483.00';
			$final_data['w30kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w30kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=483.00&amp;kg=30&amp;protocol=standard";
			
			
			$final_data['w5kg']['express']['amount'] = '110.00';
			$final_data['w5kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w5kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=110.00&amp;kg=5&amp;protocol=express";
			
			$final_data['w10kg']['express']['amount'] = '297.00';
			$final_data['w10kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w10kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=297.00&amp;kg=10&amp;protocol=express";
			
			$final_data['w15kg']['express']['amount'] = '319.00';
			$final_data['w15kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w15kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=319.00&amp;kg=15&amp;protocol=express";
			
			$final_data['w30kg']['express']['amount'] = '483.00';
			$final_data['w30kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w30kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=483.00&amp;kg=30&amp;protocol=express";
            else: 
             $final_data['w5kg']['standard']['amount'] = number_format('110.00' * 2, 2);
			$final_data['w5kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w5kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('110.00' * 2, 2)."&amp;kg=5&amp;protocol=standard";
			
			$final_data['w10kg']['standard']['amount'] = number_format('297.00' * 2, 2);
			$final_data['w10kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w10kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('297.00' * 2, 2)."&amp;kg=10&amp;protocol=standard";
			
			$final_data['w15kg']['standard']['amount'] = number_format('319.00' * 2, 2);
			$final_data['w15kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w15kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('319.00' * 2, 2)."&amp;kg=15&amp;protocol=standard";
			
			$final_data['w30kg']['standard']['amount'] = number_format('483.00' * 2, 2);
			$final_data['w30kg']['standard']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w30kg']['standard']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('483.00' * 2, 2)."&amp;kg=30&amp;protocol=standard";
			
			
			$final_data['w5kg']['express']['amount'] = number_format('110.00' * 2, 2);
			$final_data['w5kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w5kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('110.00' * 2, 2)."&amp;kg=5&amp;protocol=express";
			
			$final_data['w10kg']['express']['amount'] = number_format('297.00' * 2, 2);
			$final_data['w10kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w10kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('297.00' * 2, 2)."&amp;kg=10&amp;protocol=express";
			
			$final_data['w15kg']['express']['amount'] = number_format('319.00' * 2, 2);
			$final_data['w15kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w15kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('319.00' * 2, 2)."&amp;kg=15&amp;protocol=express";
			
			$final_data['w30kg']['express']['amount'] = number_format('483.00' * 2, 2);
			$final_data['w30kg']['express']['currency'] = 'GBP';
			$ow = str_replace('w', '', $weight);
			$final_data['w30kg']['express']['url'] = get_bloginfo('home') . "/order?cf=" . $from_country_code . "&ct=" . $to_country_code . "&amp;pr=".number_format('483.00' * 2, 2)."&amp;kg=30&amp;protocol=express";
    ...
    public function get_single_shopping_order($order_id) {
        ...
        $row->delivery_address = unserialize($row->delivery_address);
        $row->collection_address = unserialize($row->collection_do_quick_quoteaddress);
        return $row;
    }
